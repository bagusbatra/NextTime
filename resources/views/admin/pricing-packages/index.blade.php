<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Paket Harga</h2>
                <p class="text-sm text-gray-500 mt-0.5">Kelola kartu paket harga di section "Layanan Kami".</p>
            </div>
            <a href="{{ route('admin.pricing-packages.create') }}"
               class="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600 text-white text-sm font-medium rounded-lg hover:bg-cyan-700 transition">
                <i data-lucide="plus" class="h-4 w-4"></i> Tambah Paket
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <form method="GET" class="bg-white shadow-sm ring-1 ring-gray-200 rounded-xl p-4 flex flex-wrap items-end gap-3">
                ...
                <div class="w-40">
                    ...
                    <select name="tier" class="mt-1 block w-full text-sm border-gray-300 rounded-lg focus:border-cyan-500 focus:ring-cyan-500">
                ...
                <div class="flex gap-2">
                    ...
                    @if (request()->hasAny(['search', 'tier', 'status']))
                        <a href="{{ route('admin.pricing-packages.index') }}" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">Reset</a>
                    @endif
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($packages as $package)
                            <tr class="reveal hover:bg-gray-50/60 transition-colors">
                                <td class="px-6 py-3 text-gray-900 font-medium">{{ $package->name }}</td>
                                <td class="px-6 py-3 text-gray-500 capitalize">{{ $package->tier }}</td>
                                <td class="px-6 py-3 text-gray-500">
                                    {{ $package->price_prefix }} {{ $package->price_amount }}{{ $package->price_unit }}
                                </td>
                                <td class="px-6 py-3">
                                    @if ($package->is_best_seller)
                                        <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700">⭐ Best Seller</span>
                                    @else
                                        <span class="text-gray-300">—</span>
                                    @endif
                                </td>
                                <td class="px-6 py-3">
                                    <x-admin.switch :action="route('admin.pricing-packages.toggle', $package)" :on="$package->is_active" label="Aktifkan/nonaktifkan paket" />
                                </td>
                                <td class="px-6 py-3 text-right space-x-3">
                                    <a href="{{ route('admin.pricing-packages.edit', $package) }}" class="text-cyan-600 hover:underline">Edit</a>
                                    <form action="{{ route('admin.pricing-packages.destroy', $package) }}" method="POST" class="inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-gray-400">
                                    Belum ada paket harga. <a href="{{ route('admin.pricing-packages.create') }}" class="text-cyan-600 hover:underline">Tambah paket pertama</a>.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
